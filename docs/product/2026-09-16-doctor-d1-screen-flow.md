...
- **Tech 任务包 / BIM 域约束**：用户提供的任务包及对 PR #11 @ `782c63e`
  的 Tech/BIM 评审；下文按更正后的约束逐项落位。成员层已有覆盖读数，
  不再把 D1 零 finding 成员场景挂到 F1；构件层覆盖区分是否纳入产品仍待 PM 决定。
  原任务包与评审通过用户转交，尚无仓库内独立来源；本文记录设计采用的含义，
  不代替原始决策记录。`bdd67b5` 与原域约束的远端可追溯性待产品/仓库所有者处理，
  本 PR 不推送需求分支或代作产品裁定。
...
研究 disposition 不能映射成 READY 等结果。当前处置注明“BIM 研究建议，PM 尚未
裁定”，不能呈现为已定结论。候选来源、版本、适用性由 PM/BIM 提供；缺资料则标
“尚未提供”。不表示新加坡完整覆盖、CORENET X 对齐或官方批准。

### SX：真实拒绝页

主标题：“评估被拒绝 — 本次没有生成评估记录”。展示所提交上下文、实际
`PurposeAssessmentError.code` 和 Framework 提供的纯消息文本（待 F8）。当前仅有
含 code 前缀的 `str(error)`，F8 交付前可以整段显示一次原始异常文本，不做字符串
切分/去重，不从文本推断拒绝类别。随附项目当前可见码为
`team-mapping-decision-basis-illustrative`。不显示零问题、完成百分比、成员裁决，
也不把异常变成 UNKNOWN。消息按文本显示，不执行或丢弃原文。

其下的“当前版本的已知限制（说明，不是本次执行日志）”**按条件渲染**：只有
可信输入来源确认请求采用本文核对过的随附基线政策，且本次实际拒绝为
`team-mapping-decision-basis-illustrative` 时，才显示下表。仅项目名相同或异常码
相同不足以证明政策未变；基线归属未知、政策已改、其他项目或其他拒绝均不显示
此表。该条件由 Framework/接入边界提供可核依据，UI 不读取政策重做判断；
依据未交付时保持隐藏。下表不是任何项目通用的诊断组件：
...
表内固定文案：“处理当前拒绝原因不保证随后可评估；其余限制尚未由本次运行验证。”
不满足显示条件时只呈现本次异常与通用提示“本次只返回此拒绝原因，未提供其他
环节的诊断”，不能用“不能泛化”的小字替代隐藏规则。
...
覆盖说明：本绑定无finding / finding不适用 / 已评估读数 / 未请求活动
```
...
`geo-reference` 在三个活动均属 `out_of_subject_class`，只能出现在排除范围，
不加入成员场景。R-010 在本 Pack 仅作 `insufficient_evidence`，不绑定成成员读数。

构件层仍无法只凭 canonical 44 行区分“从未被规则评估”和“已评估但未被命名”，
这是 F1 的产品范围待决项，不在烟囱成员行显示“当前无法区分”。上述成员层解释
仅适用于当前 Pack/绑定；未来若把 completeness 类规则绑定为逐主体读数，需要
重新审查覆盖证据，不能把此解释无条件复用到其他 Pack。
...
- `route.next_action` 与 `route.recheck_condition` 完整可读。源定位用 canonical
  ...
  不展示假“在 Revit 打开”按钮。楼层为空显示“无楼层归属”，不作为缺失数据错误
  或模型缺陷；当前 21/44 行为空，包括设定点和屋面，与“名称不可用”分开处理。
...
已添加交叉引用或“已解决”。在 READY 旁直接注明“此结果来自穿透判定改变，
不是开洞修复完成”，并就近展示 `cause` 与 `evidence_carry_over` 的引用/变化原因，
让经理能检查这次改判的依据；不得把该反例包装成真实生产修复建议。
另一个演示是模型重发后旧对齐判定不再可归属，
...
| `PurposeAssessmentError` | SX：`.code` 与含 code 前缀的完整 `str(error)`；没有 `.message` 或独立纯消息，`.args[0]` 也是带前缀文本。结构化分栏消息待 F8，UI 不解析字符串 |
...
| --- | --- | --- |
| F1 | **构件层、范围待决**：区分“从未被规则评估”和“已评估未被命名”的版本绑定覆盖证据，canonical 行数不携带此信息 | 不阻塞 D1 的成员验收；PM 决定是否纳入构件层能力，若纳入再由 Tech 定载体。当前 Pack 的成员三态已有 reading，无需新增数据 |
...
| F4 | 多闸门完整诊断 / 执行到达信息，以及已知限制说明所需的可信基线政策归属依据 | 当前只有首个异常；SX 仅在已证实基线归属且实际拒绝匹配时显示说明，否则隐藏。自动穷举诊断未承诺，UI 不探测政策或造运行日志 |
...
| F8 | 与 code 分离、不含 code 前缀的纯消息文本；当前异常只有 `.code` 与格式化文本 | 由 Framework 暴露原始纯消息，接口形状交 Tech 决定；UI/适配器不得解析 `[code] ` 前缀来切分。交付前整段原样展示一次；本轮不实现 |
| F9 | 拒绝信封不携带请求所用政策的来源依据：只有 `mode / outcome / refusal / elements`，没有任何可核依据说明这次请求用的是随附基线政策 | 因此 SX 的"已知限制"表在本切片**不显示**——显示它的条件是"可信来源证明用的是随附基线政策"且"拒绝码匹配"，第一半无从成立。`mode === "real"` 只说明走了真实入口，不说明项目或政策，不得据此显示。不得从 `refusal.text` 嗅探项目名、不得拿 `elements` 比对 canonical、不得以任何推断补出依据。通用提示"处理当前拒绝原因不保证随后可评估"与实际拒绝照常显示。依据由 Framework/接入边界提供后再谈显示 |

新加坡候选来源与研究 disposition 由 PM/BIM 提供，不是要求 Framework 伪造
执行数据。F2–F5、F7–F9 是集成需求清单，不构成本轮批准的新接口或公共契约。

## 4. 八条域约束与评审场景

| 约束 | 屏幕落位 | 评审时应能看到的区别 |
| --- | --- | --- |
| 1 零 finding（域评审更正） | S2 成员覆盖三态、F1 构件层待决 | 本 Pack 成员三态可区分，D1 不受 F1 阻塞；构件层缺口不套到成员行 |
| 2 成员粒度 | S2 成员对表、S3 页首、S4 多对应 | 同一烟囱的 slab/roof 对各有裁决 |
| 3 排除范围 | S2 每活动表后默认可见列表 | 两个设定点有名称/类别，没有裁决 |
| 4 拒绝 | SX | 一个实际异常；仅有依据的随附基线显示已知限制表，其他情况隐藏；不承诺修好一个就可运行 |
...
边界，BIM 核成员/证据/修复/复检含义，PM 决定 F1 构件层能力是否纳入范围并审阅
经理流程。对 `782c63e` 已收到技术侧有条件通过（仅数据来源与支持边界）及 BIM
AT RISK（成员/构件混用一项）；本次修订回应这些意见，尚待复审及 PM 结论，
不得由本文宣告整体通过。后续 UI 与内部适配器需独立委派。

初版 `782c63e` 的本地工程核验记录：仓库 `.venv` 的 ruff 无错误；强制
