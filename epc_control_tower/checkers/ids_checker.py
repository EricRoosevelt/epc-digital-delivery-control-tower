...
def requirement_label(facet: object) -> str:
...
def _legacy_severity_for_rule(rule_id: str) -> Severity:
    """Severity of a failure, for rule documents that cannot state one.

    **Only reachable when loading a bare ``.ids`` file.** Declarative rules
    carry ``severity`` as a field, so the rule library no longer derives it
    from anything — R-005A and R-005B say ``severity = "WARNING"`` in their own
    files, and say why in a comment beside it.

    This survives for one document: the frozen rule set 0.1, which the legacy
    adapters publish and which is IDS all the way down. IDS 1.0 has nowhere to
    record how badly a rule matters, so a document that predates the rule
    library has nothing to read. Re-deriving the published severities is the
    only way to keep publishing them.

    It retires with the legacy adapters, and with the last bare ``.ids`` rule
    set.
    """
...
def compile_rule_directory(path: Path, *, output_dir: Path | None = None) -> IdsRuleSource:
    """Compile declarative rules into a rule source, writing the IDS document.

    The document is a build product: it is what IfcTester executes, and it is
    regenerated by every run so the same ``git diff --exit-code`` gate that
    covers the CSVs covers it too. Writing it out rather than holding it in
    memory also means the external IDS syntax audit has something to read.

    The rule set itself comes from the TOML, not from the document. IDS has
    nowhere to record who owns a requirement, at which stage it applies, or how
    badly it matters, so a round trip through the document would lose exactly
    the metadata these definitions exist to carry.
    """

    from ..rule_definitions import compile_document, load_rule_definitions

    definition = load_rule_definitions(path)
    document, ruleset, expectations = compile_document(definition)
    ...
    return IdsRuleSource(
        ...
        ruleset=ruleset,
        expectations=MappingProxyType(dict(expectations)),
...
def load_ids_rule_source(path: Path, *, ruleset_id: str = "ids") -> IdsRuleSource:
    ...
    if path.is_dir():
        return compile_rule_directory(path)
    ...
    for specification in document.specifications:
        ...
        for facet in specification.requirements:
            requirement_id = requirement_label(facet)
            ...
            requirements.append(
                Requirement(
                    ...
                    severity=_legacy_severity_for_rule(rule_id),
                )
            )
...
class IdsChecker:
    ...
    def _validate_model(self, model: Model, context: CheckContext) -> dict:
        ...
        document = ids.open(str(self.rule_source().path))
